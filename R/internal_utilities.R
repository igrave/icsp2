...
expandX <- function(formula, data, fit) {
  ...
  if (nrow(ans) != nrow(x)) {
    ans <- t(ans)
    if (nrow(ans) != nrow(x)) {
      stop('nrow(ans) != nrow(x)')
    }
    if (ncol(ans) != (ncol(x) - 1)) {
      stop('ncol(ans) != ncol(x) - 1')
    }
  }
  return(ans)
}
...
removeSurvFromFormula <- function(formula, ind = 2) {
  if (formula[[ind]][[1]] == 'Surv') {
    if (formula[[ind]][[4]] != 'interval2') {
      stop('Surv type not "interval2" in formula')
    }
...
expandY <- function(formula, data, fit) {
  if (inherits(fit, 'ic_np')) {
    if (ncol(data) != 2) {
      stop('expandY expected an nx2 matrix for data')
    }
    return(data)
  }
  if (is.null(data)) {
...
fullParamFit_exp <- function(
  ...
  if (any(data$param_y[, 2] > rightCenVal)) {
    stop(
      'finite right side of interval greater than provided rightCenVal. Use larger rightCenVal'
    )
  }
...
subSampleData <- function(data, max_n_use, weights) {
  if (nrow(data) <= max_n_use) {
    return(list(data = data, w = weights))
  }
  if (is.null(weights)) {
...
removeVarFromCall <- function(call, varName) {
  ...
  if (inherits(newcall, 'try-error')) {
    warnMessage <- paste(
      'Unable to determine formula for splitting on variable  ',
      varName,
      #		'Original formula = ', call,
      '\nAttempted formula after removing variable = ',
      pastedCall,
...
s_exp <- function(x, par) {
  return(1 - pexp(x, exp(-par[1])))
}
s_weib <- function(x, par) {
  return(1 - pweibull(x, exp(par[1]), exp(par[2])))
}
s_gamma <- function(x, par) {
  return(1 - pgamma(x, shape = exp(par[1]), scale = exp(par[2])))
}
s_lnorm <- function(x, par) {
  return(1 - pnorm(log(x), mean = par[1], sd = exp(par[2])))
}
...
get_etas <- function(fit, newdata = NULL, reg_pars = NULL) {
  if (fit$par == 'non-parametric') {
    ans <- 1
    names(ans) <- 'baseline'
    return(ans)
  }
  if (is.null(reg_pars)) {
    reg_pars <- default_reg_pars(fit)
  }
  if (!is.matrix(reg_pars)) {
    reg_pars <- matrix(reg_pars, ncol = length(default_reg_pars(fit)))
  }
  if (is.null(newdata)) {
    ans <- 1
    names(ans) <- 'baseline'
  ...
  if (identical(rownames(newdata), NULL)) {
    rownames(newdata) <- as.character(1:icr_nrow(newdata))
  }
  ...
  etas <- exp(log_etas)
  if (length(grpNames) == length(etas)) {
    names(etas) <- grpNames
  }
  return(etas)
}
...
po_link <- function(s, nu) {
  nu * s / (s * (nu - 1) + 1)
}
ph_link <- function(s, nu) {
  s^nu
}
no_link <- function(s, nu) {
  s
}
...
findUpperBound <- function(
  ...
  if (fval < x) {
    stop('finding upper bound for quantile failed!')
  }
  return(val)
}
...
getFormula <- function(object) {
  if (inherits(object, 'formula')) {
    return(object)
  } else if (inherits(object, 'icenReg_fit')) {
...
getData <- function(fit) {
  ...
  if (is.null(ans)) {
    stop(
      'Could not find data from fit. Original model must be built with data argument (rather than variables found in the Global Environment) supplied to be retreivable'
    )
  }
  return(ans)
}
...
get_tbull_mid_q <- function(p, s_t, tbulls) {
  if (any(is.na(c(p, s_t, tbulls)))) {
    stop('NAs provided to get_tbull_mid_q')
  }
  x_low <- tbulls[, 1]
  x_hi <- tbulls[, 2]
  ...
  for (i in seq_along(p)) {
    ...
    if (this_p < 0 | this_p > 1) {
      stop('attempting to find survival quantile for invalid probability')
    }
    if (this_p == 0) {
      ans[i] <- x_range[1]
    } else if (this_p == 1) {
...
PCAFit2OrgParFit <- function(PCA_info, PCA_Hessian, PCA_parEsts, numIdPars) {
  ...
  if (tot_k != length(PCA_parEsts)) {
    stop('incorrect dimensions for PCAFit2OrgParFit')
  }
  if (numIdPars == 0) {
    pcaTransMat <- PCA_info$rotation
  } else {
  ...
  for (i in seq_along(PCA_info$scale)) {
    pcaTransMat[i + numIdPars, ] <- pcaTransMat[i + numIdPars, ] /
      PCA_info$scale[i]
  }
  ans <- list()
...
getSurvProbs <- function(times, etas, baselineInfo, regMod, baseMod) {
  ...
  if (is.null(regInt)) {
    stop('regMod type not recognized')
  }
...
getSamplableVar <- function(fit) {
  ...
  if (is.null(ans)) {
    stop(
      'coefficient variance not found for fit. If ic_ph model was fit, make sure to set bs_samples > 100 to get bootstrap sample of variance'
    )
  }
  return(ans)
}
...
setSamplablePars <- function(fit, coefs) {
  if (!inherits(fit, 'ic_np')) {
    ...
    if (length(coefs) > n_base) {
      fit$reg_pars[1:(length(coefs) - n_base)] <- coefs[
        (n_base + 1):length(coefs)
      ]
    }
  }
}
...
fastNumericInsert <- function(newVals, target, indices) {
  if (storage.mode(newVals) != 'double') {
    storage.mode(newVals) <- 'double'
  }
  if (storage.mode(target) != 'double') {
    stop('target of fastNumericInsert MUST have storage.mode = "double"')
  }
  if (storage.mode(indices) != 'integer') {
...
fastMatrixInsert <- function(newVals, targMat, rowNum = NULL, colNum = NULL) {
  if (is.null(colNum)) {
    if (is.null(rowNum)) {
      stop('need either rowNum or colNum')
    }
...
updateDistPars <- function(vals, max_n) {
  ...
  if (n_v != max_n) {
    stop('parameters are not of equal length (or 1)')
  }
  return(vals)
}
...
getMaxLength <- function(thisList) {
  n <- 0
  for (i in seq_along(thisList)) {
    n <- max(c(n, length(thisList[[i]])))
  }
  return(n)
}
...
addListIfMissing <- function(listFrom, listInto) {
  ...
  for (n in listFromNames) {
    listInto <- addIfMissing(listFrom[[n]], n, listInto)
  }
  return(listInto)
}
...
readingCall <- function(mf) {
  m <- match(
    c("formula", "data", "subset", "na.action", "offset"),
    names(mf),
    0L
  )
  mf <- mf[c(1L, m)]
...
checkStrata <- function(strata, yMat) {
  if (is.null(strata)) {
    return(factor(rep(1, nrow(yMat))))
  }
  if (length(strata) != nrow(yMat)) {
    stop('strata must be same length as y')
  }
  if (any(is.na(strata))) {
    stop('NAs not allowed in strata')
  }
...
sample_etas_and_base <- function(fit, samples, newdata) {
  ...
  if (nBase == 1) {
    basePars <- as.matrix(basePars, ncol = 1)
  }
  ...
  ans <- list(baseMat = basePars, etas = etas)
  return(ans)
}
