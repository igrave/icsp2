...
getSCurves.default <- function(fit, newdata = NULL) {
  if (inherits(fit, 'impute_par_icph')) {
    stop('getSCurves currently not supported for imputation model')
  }
  if (inherits(fit, 'ic_par')) {
    stop(
      'getSCurves does not support ic_par objects. Use getFitEsts() instead. See ?getFitEsts'
    )
  }
...
simICPO_beta <- function(
  ...
  obsTimes <- 1 /
    (inspections + 1) +
    runif(n, min = -inspectionError, max = inspectionError)
  if (!is.null(rndDigits)) {
    obsTimes <- round(obsTimes, rndDigits)
  }
...
diag_covar <- function(
  ...
  if (!yType %in% c('survival', 'transform', 'meanRemovedTransform')) {
    stop(
      "yType not recognized. Options = 'survival', 'transform' or 'meanRemovedTransform'"
    )
  }
  if (missing(data)) {
    if (!is(object, 'icenReg_fit')) {
      stop("either object must be icenReg_fit, or formula with data supplied")
    }
  ...
  if (is.null(model)) {
    stop('either object must be a fit, or model must be provided')
  }
  if (missing(data)) {
    data <- getData(object)
  }
  ...
  if (model == 'aft') {
    stop(
      'diag_covar not supported for aft model. This is because calculating
  ...
  if (is.null(vals)) {
    stop(paste('Cannot find variable', varName, 'in original dataset'))
  }
  orgCall <- fullFormula
  newcall <- removeVarFromCall(orgCall, varName)
  if (identical(orgCall, newcall)) {
    stop('varName not found in original call')
  }

  spltInfo <- NULL
  if (factorSplit) {
    factVals <- factor(vals)
    if (length(levels(factVals)) > 20) {
      stop(
        'Attempting to split on factor with over 20 levels. Try using numeric version of covariate and use numericCuts instead'
      )
    }
    spltInfo <- makeFactorSplitInfo(vals, levels(factVals))
  } else {
    if (missing(numericCuts)) {
      numericCuts <- median(data[[varName]])
    }
    spltInfo <- makeNumericSplitInfo(vals, numericCuts)
  }

  spltFits <- splitAndFit(
    newcall = newcall,
    data = allData,
    varName = varName,
    splitInfo = spltInfo,
...
diag_baseline <- function(
  ...
  if (model == 'aft') {
    stop(
      'diag_baseline not supported for aft model. This is because calculating
	       the non-parametric aft model is quite difficult'
    )
  }
  newdata = NULL
  ...
  legend(
    lgdLocation,
    legend = c('Semi-parametric', dists),
    col = c('black', cols),
    lwd = 1
  )
}
...
sampleSurv_slow <- function(fit, newdata, p = NULL, q = NULL, samples = 100) {
  if (nrow(newdata) > 1) {
    stop('newdata must be a single row')
  }
  ...
  if (is_np) {
    stop("Can only sample survival estimates for parametric/Bayesian models")
  }
  isBayes <- is(fit, 'bayes_fit')
  ...
  for (i in 1:samples) {
    ...
    setSamplablePars(fit, sampledCoefs)
    if (input_type == 'p') {
      theseSamples <- getFitEsts(fit, newdata, p = p)
    } else {
      theseSamples <- getFitEsts(fit, newdata, q = q)
    }
    ans[i, ] <- theseSamples
    setSamplablePars(fit, orgCoefs)
  }
  if (input_type == 'p') {
    these_colnames <- paste("p =", p)
  } else {
    these_colnames <- paste("t =", q)
  }
  return(ans)
}
...
sampleSurv <- function(fit, newdata = NULL, p = NULL, q = NULL, samples = 100) {
  ...
  isBayes <- is(fit, 'bayes_fit')
  ans <- matrix(nrow = samples, ncol = nCol)
  etas_and_base <- sample_etas_and_base(
    fit,
    samples = samples,
    newdata = newdata
  )
  ...
  if (length(etas) == 1) {
    etas <- rep(etas, samples)
  }
  baseMat <- etas_and_base$baseMat
  if (nrow(baseMat) != samples) {
    stop("nrow(baseMat) != samples")
  }
  if (length(etas) != samples) {
    stop("length(etas) != samples")
  }
  for (i in 1:nCol) {
    this_input = rep(input[i], samples)
    if (input_type == 'p') {
      ans[, i] <- computeConditional_q(
        this_input,
        etas,
        baseMat,
        fit$model,
  ...
  if (input_type == 'p') {
    these_colnames <- paste("Q(p) =", input)
  } else {
    these_colnames <- paste("F(t) =", input)
  }
...
#'
#' @export
cs2ic <- function(time, eventOccurred) {
  if (!all(eventOccurred == 0 | eventOccurred == 1)) {
    stop("eventOccurred must be all 1's and 0's or TRUE's and FALSE's")
  }
  if (length(time) != length(eventOccurred)) {
    stop('length(time) != length(eventOccurred)')
  }
  l = rep(0, length(time))
...
survCIs <- function(
  ...
  if (!is.null(p) & !is.null(q)) {
    stop('Need to provide p OR q, not both')
  }
  if (!(inherits(fit, 'par_fit') | inherits(fit, 'bayes_fit'))) {
